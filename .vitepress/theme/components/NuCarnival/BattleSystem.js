// NuCarnival Attack Calculator - Battle System
// Ported from https://github.com/linecross/nuCarnivalAttackCalculator

import {
  Character, Class, Element, Rarity, PotentialType,
  RuleType, RuleValueByType, AttackType, ConditionType,
  TargetType, ActionPattern, ALWAYS_EFFECTIVE, GAME_CONFIG
} from './Constants.js';
...
export class Condition {
  static CHECK_IN_BATTLE_LIST = [ConditionType.isAttackType, ConditionType.isAttack, ConditionType.everyTurn, ConditionType.atTurn];
  static IS_HP_FULFILL = true;
...
export class Rule {
  static idCounter = 0;
  static CHILD_RULE_ID_INCREMENT = 300000;
  ...
  toString() {
    ...
    if (this.maxCount > 1) {
      s += '（最大' + this.maxCount + '層）';
    }
    return s;
  }

  clone() {
    var cloneRule = new Rule({
      id: this.id, isPassive: this.isPassive, type: this.type, value: this.value, valueBy: this.valueBy,
  ...
  cloneSimple() {
    var cloneRule = new Rule({
      id: this.id, isPassive: false, type: this.type, value: this.value, valueBy: this.valueBy,
      turn: this.turn, maxCount: this.maxCount, condition: this.condition, target: this.target
    });
    if (this.condition != null) {
      cloneRule.condition = this.condition;
...
export class Battle {
  ...
  static OUTPUT_TYPES = [RuleType.attack, RuleType.poisonAttack, RuleType.heal, RuleType.continueHeal, RuleType.support];
  static TEAM_BUFF_TYPES = [RuleType.atkUp, RuleType.hpUp, RuleType.basicAtkUp, RuleType.skillAtkUp, RuleType.allAtkUp, RuleType.poisonAtkUp, RuleType.healUp, RuleType.continueHealUp];
  static ENEMY_BUFF_TYPES = [RuleType.enemyBasicAtkUp, RuleType.enemySkillAtkUp, RuleType.enemyAllAtkUp, RuleType.enemyPoisonAtkUp];
  ...
  getPostAttackRules(rules) {
    return rules.filter(r => r.isPostAttackRule());
  }
  ...
  startRound(attackType, card, currentTurn) {
    ...
    // Pre-attack rules (check in battle)
    for (var rule of this.getPreAttackRules(this.battleTurns[card.name].rules)) {
      if (!rule.isRuleCheckInBattle()) continue;
      if (!rule.isConditionsFulfilled(card, this.team, attackType, currentTurn)) continue;
    ...
    // Pre-attack rules (not check in battle)
    for (var rule of this.getPreAttackRules(this.battleTurns[card.name].rules)) {
      if (rule.isRuleCheckInBattle()) continue;
      if (!rule.isConditionsFulfilled(card, this.team, attackType, currentTurn)) continue;
      ...
      for (var i = 0; i < applyCount; i++) {
        if (rule.type == RuleType.atkUp) {
          ...
          this.enemyBattleTurn.addRule(rule.cloneSimple());
      ...
      for (var rule of attackRule) {
        ...
        if (Battle.OUTPUT_TYPES.includes(rule.type)) {
          ...
          if (rule.type == RuleType.attack) {
            ...
            // Post-attack team rules
            for (var postRule of this.getPostAttackRules(this.battleTurns[card.name].rules)) {
              if (!postRule.isConditionsFulfilled(card, this.team, attackType, currentTurn)) continue;
